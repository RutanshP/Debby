...
_STRUCTURE_SYSTEM_PROMPT = (
    ...
    "advantage using AFF-style solvency links, but the counterplan must be "
    "mutually exclusive with the AFF plan or topic. It must prove the AFF "
    "plan and NEG counterplan cannot happen at the same time. If the "
    "alternative can coexist with the AFF plan/topic, do not present it as "
    "a counterplan; use a status quo disadvantage instead. The counterplan "
    "must identify the specific legal, procedural, funding, jurisdictional, "
    "or implementation conflict that prevents doing both. Merely saying "
    "'instead of' or 'rather than' is not mutual exclusivity. The counterplan "
    "also must not contradict the NEG disadvantages to the AFF plan."
)
...
_PARLI_OUTPUT_RULES = (
    ...
    "    `L1 renewables: AFF plan passes → more money available for renewable energy → US spending on renewables increases`\n"
    "    `- IL1 - Emissions reduced: more renewable funding → faster clean-energy deployment → emissions fall`\n"
    "    `- IL2 - International projects: US invests in renewables → other countries collaborate with the US → global climate policy improves`\n"
    "    `- IL3 - Spillover: US investment sets precedent → allies follow US investment → worldwide renewable deployment rises`\n"
    "    `- IL4 - Lower prices: US investment increases competition → quality improves and prices fall → renewables become more accessible worldwide`\n"
    "  - **Impacts:** write dense casefile-style impact paragraphs. You do not need to separately label probability/magnitude/timeframe/reversibility for every contention, but each impact section should still naturally explain scope, severity, likelihood, timeframe when relevant, and weighing. It is acceptable to have multiple terminal impacts in one section, such as Climate Change and Poverty, when the link story accesses both.\n"
    ...
    "- If NEG runs a counterplan, include a short counterplan text, a line labeled **Mutual exclusivity:** proving why the AFF plan/topic and NEG counterplan cannot happen at the same time, solvency, and optionally one CP advantage. This proof must name the specific legal, procedural, funding, jurisdictional, or implementation conflict that makes doing both impossible. The counterplan MUST be mutually exclusive with the AFF plan/topic: if it can coexist with the AFF plan/topic, it is not allowed as a counterplan. Do not treat phrases like \"instead of\" or \"rather than\" as proof of mutual exclusivity. For example, if the AFF bans a party, a counterplan to fund civic education or political education is invalid unless you prove the education program legally or practically prevents the ban, because both policies can normally happen together. The CP advantage can use AFF-style links showing how the counterplan solves a harm, but keep it logically consistent with the disadvantages against AFF.\n"
...
_PARLI_TULI_CONTENTION_TEMPLATE = (
    ...
    "Impacts { write dense casefile-style impact paragraphs that explain scope, severity, likelihood, timeframe when relevant, and weighing. }\n"
)
...
_PARLI_AFF_USER = (
    ...
    "Weighing Mechanism { a weighing mechanism is a philosophical framework by which to judge the debate round. This is generally defaulted to net benefits { net benefits means the greatest amount of good for the greatest amount of people } under a utilitarian philosophical framework but can be changed by the debaters if they choose to set an alternative weighing mechanism. An example alternative weighing mechanism is biocentric net benefits under a utilitarian framework which means that the debate centers around the greatest amount of good for the greatest amounts of organisms on the planet. }\n"
    "Definitions/Background: { here you may define any ambiguous terms in the resolution or any terms that may be relevant to your debate case that you may reference many times. This often includes defining the scope of policy actions indicated in the resolution, or simply just ambiguous terms. Additionally, this is the place to add context for the resolution. }\n"
    "Plan Text: { Here, you may define a counter plan that the negation may choose to plan. This should only be included if the round type is a policy round AND if the topic is favorable to a counter plan. This plan MUST satisfy the resolution and be topical. This counterplan MUST be mutually exclusive with the passage of the resolution as a plan/topic. You must prove the AFF plan/topic and NEG counterplan cannot happen at the same time by naming a specific legal, procedural, funding, jurisdictional, or implementation conflict; if they can coexist, it is not allowed as a counterplan. }\n"
    "- Agent of Actor: { here, specify an entity that will pass the plan text that is specified above. (e.g. if plan text is “USFG should implement a UBI,” the agent of actor could be the United States Federal Government) }\n"
    "- Agent of Enforcer: { here, specify which entity will be in charge of enforcing/regulating this policy. (e.g. if the plan is the USFG should reduce illegal immigrants, the agent of enforcer would be ICE) } \n"
    "- Time frame: { here, specify when the policy will be implemented. Default is as soon as possible unless there is an advantage for a different time frame. }\n"
    "- Funding: { here specify the funding required in this policy. Additionally if needed, add where the money is coming from if it is a significant amount of money. Otherwise, say Normal Ways and Means (NWAM) }\n"
