...
export function ClassesClient() {
  ...
  useEffect(() => {
    getBrowserSupabase()
      ...
      .catch(() => {
        // Best effort only; comments still render without author-specific actions.
      });
  }, []);
  ...
  useEffect(() => {
    ...
    setError(queryError instanceof Error ? queryError.message : "Failed to load classroom");
  ...
  const unfinishedCount = useMemo(
    () => studentAssignments.filter((item) => item.recipient.status !== "completed").length,
    [studentAssignments],
  );
  ...
  const calendarItems = useMemo(() => {
    if (!classDetail) return [];
    return classDetail.role === "coach" ? coachAssignments : studentAssignments;
  }, [classDetail, coachAssignments, studentAssignments]);
  ...
  useEffect(() => {
    ...
    if (classDetail?.role === "competitor") {
      setTab("stream");
      return;
    }
    setTab("classwork");
            ...
            <section className="flex flex-col gap-5">
              ...
              <div>
                {classDetail.role === "competitor" && (
                  <nav aria-label="Classroom sections" className="mb-5 flex flex-wrap gap-2">
                    {[
                ...
                {classLoading ? (
                  ...
                  <div className="flex flex-col gap-5">
                    {classDetail.role === "competitor" && (
                      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
                        <UpcomingCard assignments={studentAssignments} />
                  ...
                  <div className="rounded-lg border border-slate-200 bg-slate-50 p-5">
                    <p className="text-sm text-slate-600">
                      Progress view is available for coaches only.
                    </p>
                  </div>
                ) : classDetail.role === "coach" ? (
