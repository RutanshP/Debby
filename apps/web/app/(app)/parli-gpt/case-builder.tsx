...
const evidenceIssueStats = [
  ...
  {
    label: "Flagged for misrepresentation",
    value: "1",
    tone: "bg-amber-400 text-slate-950",
  },
];

function EvidenceIssuesCard() {
  return (
    <section className="mt-8 rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 via-white to-teal-50 p-5 shadow-sm">
      <h2 className="mb-4 text-xl font-semibold text-slate-900">Evidence Issues</h2>
      <div className="grid gap-3 md:grid-cols-3">
        {evidenceIssueStats.map((item) => (
          <div
            key={item.label}
          ...
          >
            ...
            <div className="mt-3 flex items-end justify-between gap-3">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ${item.tone}`}>
                {item.label === "Flagged for misrepresentation" ? "Review needed" : "Verified"}
              </span>
            </div>
          </div>
        ))}
      ...
      <div className="mt-4 rounded-2xl border border-amber-100 bg-white/80 p-4">
        <p className="text-sm leading-7 text-slate-700">
          Evidence scan found one citation that overstates what the underlying stat
          proves, while the remaining twenty-one evidence references appear internally
        ...
        <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50/80 p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-sm font-semibold text-slate-900">
              &quot;99% of economy is small businesses&quot;
            </span>
          </div>
          <p className="mt-3 text-sm leading-7 text-slate-700">
            This was flagged for misrepresentation. The underlying statistic is closer
            to 99.9% of all firms being small businesses, but that does not mean small
            businesses make up 99% of GDP. Their actual share of total economic output
            is under 50%, so the citation overclaims what the source supports.
          </p>
        </div>
        <div className="mt-3 flex flex-wrap gap-2">
          <span className="rounded-full bg-teal-50 px-3 py-1 text-xs font-medium text-teal-800">
            21 evidence references verified
          </span>
          <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">
            1 statistic flagged for scope inflation
          </span>
        </div>
      </div>
    </section>
